...
```{r}
quantitative_data <- (Stroke[,c(2:12)])
...
```
#### In the histograms, we see that the average age of the customer sample is 45. The average professional experience is 20 years. The average income is 65'000 $ annually. The average spending on Credit Card is 1'500$ monthly. As for the morgage expenses we see that the biggest category do not have any (median result & histogram visually). However the mean is aroung 50'000$. Income & CCAvg are a little bit right skewed, meaning that the median is a little smaller than the mean (the above analysis is based on the mean). We must be aware of that because it changes the distribution and maybe looking at the median is more relavant than the mean
