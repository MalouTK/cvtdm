...
#### Preprocessing the data (preparing for analysis) : 
...
### Normalizing the dataset for KNN and classification tree:
```{r}
...
### Transforming the categorical variables with more than 2 categories into dummy variables (Here it concerns variables work_type & smoking_status)

```{r}
set.seed(1)
Stroke.norm.dummy = Stroke_normalized
# Put work_type as dummy:
work_type_Dummies <- cbind( Stroke.norm.dummy[1:5], dummy(Stroke.norm.dummy$work_type, sep = "_"), Stroke.norm.dummy[7:11])
names(work_type_Dummies)[6:10] <- c("children","Govt_job", "Never_Worked", "Private", "Self-employed")
work_type_Dummies
Stroke.norm.dummy <- work_type_Dummies

# Put smoking_status as dummy:
smoking_status_Dummies <- cbind( Stroke.norm.dummy[1:13], dummy(Stroke.norm.dummy$smoking_status, sep = "_"), Stroke.norm.dummy[15])
names(smoking_status_Dummies)[14:17] <- c("formerly_smoked", "never_smoked", "smokes", "unknown")
smoking_status_Dummies
Stroke.norm.dummy <- smoking_status_Dummies

# Set training and validation (normalized + dummy) set for KNN:
set.seed(1)
Train.norm.dummy <- sample(row.names(Stroke.norm.dummy), 0.6*dim(Stroke.norm.dummy)[1])
Valid.norm.dummy <- setdiff(row.names(Stroke.norm.dummy), Train.norm.dummy)
Train.norm.dummy <- Stroke.norm.dummy[Train.norm.dummy, ]
Valid.norm.dummy <- Stroke.norm.dummy[Valid.norm.dummy, ]
dim(Train.norm.dummy)
dim(Valid.norm.dummy)

# KNN tolerates no missing values (here: in bmi): omit them
Train.norm.dummy <- na.omit(Train.norm.dummy)
Valid.norm.dummy <- na.omit(Valid.norm.dummy)
```

```{r}
# KNN
knn(Train.norm.dummy[,-18], Valid.norm.dummy[,-18], cl = Train.norm.dummy[,18], k = 3, prob = T)
...
````{r}
# Dummify:
Stroke$stroke <- as.integer(ifelse(Stroke$stroke == "0", 0,1))  # change stroke to int for dummification
...
train.index = createDataPartition(Stroke.fltr$stroke, p = .6, list = FALSE)
train.set = Stroke.fltr[ train.index,]
val.set  = Stroke.fltr[-train.index,]
print(table(train.set$stroke) / table(Stroke.fltr$stroke)) #Training set partition
print(table(val.set$stroke) / table(Stroke.fltr$stroke)) #Validation set partition
```

```{r}
train.norm.df = train.set
valid.norm.df = val.set
# Normalization of numerical variables:
norm.values = preProcess(train.set[, c(4,15,16)], method=c("center", "scale"))
train.norm.df[, c(4,15,16)] = predict(norm.values, train.set[, c(4,15,16)])
valid.norm.df[, c(4,15,16)] = predict(norm.values, val.set[, c(4,15,16)])

train.set.num = train.set[, c(4,15,16)] 
train.set.num.melt = melt(train.set.num)

train.set.num.boxplot = ggplot(train.set.num.melt, aes(y=value, Stroke$stroke)) + 
  geom_boxplot() + 
  facet_wrap(~variable, scales = "free", ncol=3) + theme_classic() + xlab("Variables")

#train.set.num.boxplot
```
...
nk = 10
metrics.df2 <- data.frame(k = seq(1, nk, 1), accuracy = rep(0, nk), sensitivity = rep(0, nk), F1_score = rep(0, nk))

for(i in 1:nk) {
  knn.pred = knn(train.norm.df[, -21], valid.norm.df[, -21], cl = train.norm.df[, 21], k = i)
  metrics.df2[i, 2] = confusionMatrix(knn.pred, valid.norm.df[, 21], positive = "1")$overall[1]
  metrics.df2[i, 3] = confusionMatrix(knn.pred, valid.norm.df[, 21], positive = "1")$byClass[1]
}

accuracy.plot = ggplot(data= metrics.df2) + geom_line(aes(x=k,y=accuracy), color="darkred") + theme_classic()
sensitivity.plot = ggplot(data= metrics.df2) + geom_line(aes(x=k,y=sensitivity), color="blue") + theme_classic()
both.plot = ggplot(data= metrics.df2) + geom_line(aes(x=k,y=accuracy), color="darkred") + geom_line(aes(x=k,y=sensitivity))+ ylab("Both") + theme_classic()
...
k.best = metrics.df2$k[which.min(metrics.df2$difference)]
k.best
```
...
knn.pred = knn(train.norm.df[, -21], valid.norm.df[, -21], cl = train.norm.df[, 21], k = k.best) 
## k.best = 1:
confusionMatrix(knn.pred, valid.norm.df[, 21], positive = "1")
fourfoldplot(confusionMatrix(knn.pred, valid.norm.df[, 21], positive = "1")$table)
```
